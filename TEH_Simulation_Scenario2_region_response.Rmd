...
X$Xregion<-sample(c("N", "O"), size = 100, replace = TRUE, prob = c(0.75, 0.25))
...
#We have b1*=1.147860.Then we consider five situations: b1=0,b1=0.5*(b1*),b1=1*(b1*),b1=1.5*(b1*),b1=2*(b1*).
```

```{r cleaning, echo=FALSE}
#We simulate b1=1*(b1*).
get_b0(X, scal, prog, pred, trt,
       b1 = 1.147860*2, type = "continuous",
       power = 0.5, alpha = 0.025, interval = c(-2, 2), sigma_error = 1
)
#We have b0=-0.511795.

#Calculate response variable based on b0 and b1
dat <- generate_y(X, trt, prog = "1.7*1.28*(X31)",
                  pred = "X31", b0 = -0.511795, b1 =1.147860*2,
...
ggsave("/Users/tianyuzheng/Documents/PKU RA/Novartis/Figures/var_importance_scene2_region_response.png", p_importance, width = 6, height = 4, units = "in")
