...
region_counts <- rmultinom(n = 1, size = 500, prob = probs)
region_labels <- rep(names(probs), times = region_counts)

# Save region labels to X
X$region <- region_labels
...
```{r} 
#Strong U-Region Association

# Assign U based on region
X$U <- NA

# Assign U based on region
X$U[X$Xregion == "N"] <- sample(c(0, 1), size = sum(X$Xregion == "N"), replace = TRUE, prob = c(0.95, 0.05))
X$U[X$Xregion == "O"] <- sample(c(0, 1), size = sum(X$Xregion == "O"), replace = TRUE, prob = c(0.05, 0.95))
# Optionally make it a factor
#X$U <- factor(X$U, levels = c(0, 1))

table(X$U, X$Xregion)
prop.table(table(X$U, X$Xregion), margin = 2)  # Check proportions by region

ggplot(X, aes(x = factor(X31), fill = factor(U))) +
...
```{r}
#Weak U-Region Association

# Set overall probability of U = 1 (e.g., 0.5)
X$U <- rbinom(n = nrow(X), size = 1, prob = 0.5)

# Optionally convert to factor
#X$U <- factor(X$U, levels = c(0, 1))

table(X$U, X$Xregion)
prop.table(table(X$U, X$Xregion), margin = 2)  # Proportions of U within each region
...
#We have b1*=0.3024111.Then we consider five situations: b1=0,b1=0.5*(b1*),b1=1*(b1*),b1=1.5*(b1*),b1=2*(b1*).
```

```{r cleaning, echo=FALSE}
#We simulate b1=2*(b1*).
get_b0(X, scal, prog, pred, trt,
       b1 = 0.50178371*2, type = "continuous",
       power = 0.5, alpha = 0.025, interval = c(-2, 2), sigma_error = 1
)
#We have b0=-0.2016408.
```

```{r}
#Calculate response variable based on b0 and b1
dat <- generate_y(X, trt, prog ,
                  pred , b0 =-0.06471698, b1 =0.50178371*2,
...
ggsave("/Users/tianyuzheng/Documents/PKU RA/Novartis/Figures/var_importance_scene2_treatment_effect_response.png", p_importance, width = 6, height = 4, units = "in")
...
ggsave("/Users/tianyuzheng/Documents/PKU RA/Novartis/Figures/correlation_graph.png", p_correlation, width = 14, height = 14, units = "in")
```

```{r pressure, echo=FALSE}
var_importance_beta=2.0
################################
