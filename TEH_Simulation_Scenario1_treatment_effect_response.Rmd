...
```{r}
# Assign X2(X32) based on region
#X$X32[X$Xregion == "N"] <- sample(c(0, 1), size = sum(X$Xregion == "N"), replace = TRUE, prob = c(0.95, 0.05))
#X$X32[X$Xregion == "O"] <- sample(c(0, 1), size = sum(X$Xregion == "O"), replace = TRUE, prob = c(0.05, 0.95))
...
ggsave("/Users/tianyuzheng/Documents/PKU RA/Novartis/Figures/var_importance.png", p_importance, width = 6, height = 4, units = "in")
...
vars_to_plot <- importance_scores %>%
  arrange(desc(vi)) %>%             # sort by descending vi
  slice_head(n = 5) %>%             # take top 5
  mutate(rank = row_number()) %>%   # add rank column
  select(rank, variable, vi)        # reorder columns if you like
